...
public class DlgMedicina extends JDialog implements ActionListener, KeyListener, MouseListener {
	...
	void ajustarAnchoColumnas() {
		...
		tcm.getColumn(0).setPreferredWidth(anchoColumna(10));  // codigoMedicina
		tcm.getColumn(1).setPreferredWidth(anchoColumna(35));  // nombre
		tcm.getColumn(2).setPreferredWidth(anchoColumna(25));  // laboratorio
	...
	void listar() {
		...
		for (int i=0; i<am.tamaño(); i++) {
			...
			Object[] fila = { m.getCodigoMedicina(),
					          ...
					          m.getLaboratorio(),
					          m.getStock(),
					          "S/ " + Formato.ajusteA(m.getPrecioUnitario()) };
			modelo.addRow(fila);
		}
		if (am.tamaño() > 0)
	...
	void editarFila() {
		if (am.tamaño() == 0)
			...
			txtPrecio.setText(Formato.ajusteA(m.getPrecioUnitario()));
		}
	}
	void limpieza() {
